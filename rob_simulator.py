...
def plot_movement(world_state, obstacles, goal_point, robot_init):
...
def random_planner(x, y, new_x, new_y):
    """
    ...
    for i in range(max_step_number):
        ...
        if (new_x, new_y) not in world_state:
            new_x = x
            new_y = y
        # obstacle avoidance
            ...
            new_y = y
        # goal point reached
        elif new_x == goal_point[0] and new_y == goal_point[1]:
            print("goal point reached in {} iterations".format(i))
            # return if goal point reached
            return
        new_pos = (new_x, new_y)
        plot_movement(world_state, obstacles, goal_point, new_pos)
        plt.pause(pause_sec)
...
def run_random_approach():
    plot_movement(world_state, obstacles, goal_point, robot_init)
    random_planner(x=robot_init[0], y=robot_init[1],
                   new_x=robot_init[0], new_y=robot_init[1])


def run_optimal_approach():
    """ optimal_planner """
